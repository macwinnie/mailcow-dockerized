...
while (($#)); do
  case "${1}" in
    --check|-c)
      ...
      LATEST_REV=$(git ls-remote --exit-code --refs --quiet https://github.com/macwinnie/mailcow-dockerized ${BRANCH} | cut -f1)
      if [ $? -ne 0 ]; then
        echo "A problem occurred while trying to fetch the latest revision from github."
        exit 99
      fi
      if [[ -z $(git log HEAD --pretty=format:"%H" | grep "${LATEST_REV}") ]]; then
        echo -e "Updated code is available.\nThe changes can be found here: https://github.com/macwinnie/mailcow-dockerized/commits/master"
